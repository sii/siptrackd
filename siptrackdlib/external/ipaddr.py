...
def IPNetwork(address, version=None, strict=False, mask_address=False):
    """Take an IP string/int and return an object of the correct type.
    ...
    if version:
        if version == 4:
            return IPv4Network(address, strict, mask_address)
        elif version == 6:
            return IPv6Network(address, strict, mask_address)
...
class IPv4Network(_BaseV4, _BaseNet):
    ...
    def __init__(self, address, strict=False, mask_address=False):
        ...
        tuple_prefixlen = None
        if type(address) in [list, tuple] and len(address) == 2:
            address, tuple_prefixlen = address

        # Efficient constructor from integer.
        if isinstance(address, (int, long)):
            self._ip = address
            self.ip = IPv4Address(self._ip)
            if tuple_prefixlen is None:
                self._prefixlen = self._max_prefixlen
                self.netmask = IPv4Address(self._ALL_ONES)
            else:
                self._prefixlen = int(tuple_prefixlen)
        ...
        if mask_address:
            if self.ip != self.network:
                self.ip = IPv4Address(self.network._ip)
        if strict:
            if self.ip != self.network:
...
class IPv6Network(_BaseV6, _BaseNet):
    ...
    def __init__(self, address, strict=False, mask_address=False):
        ...
        if mask_address:
            if self.ip != self.network:
                self.ip = IPv6Address(self.network._ip)
        if strict:
            if self.ip != self.network:
                raise ValueError('%s has host bits set' %
                                 self.ip)
